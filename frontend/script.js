document.addEventListener('DOMContentLoaded', function() {
    ...
    const BACKEND_URL = 'https://expo-62r9.onrender.com';
    ...
    cameraBtn.addEventListener('click', () => {
        // Si ya subió una foto, no permitir otra
        if (localStorage.getItem('foto_subida')) {
            alert('Solo puedes subir una foto por dispositivo.');
            return;
        }
        fileInput.value = '';
        fileInput.click();
        ...
        if (this.files && this.files[0]) {
            ...
            .then(data => {
                if (data.ok && data.filename) {
                    ...
                    // Guardar en localStorage que ya subió una foto
                    localStorage.setItem('foto_subida', '1');
                }
            });
        }
    });
